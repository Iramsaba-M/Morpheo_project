...
export const Domain = ({ elements, onNodeClick, selectedNodes }) => {
  ...
  useEffect(() => {
    ...
    haloDiv.style.backgroundColor = "#D9D9FF";
    ...
    centerIcon.innerHTML = `<img src="/images/graph/boxes.svg" alt="domains" style="width: 50px; height: 50px; filter: brightness(0) invert(1);">`;
    overlayDiv.appendChild(centerIcon);

    // Filter out edges and transform elements
    const transformedElements = elements
      .filter(item => {
        // Only keep domain nodes
        return item.data.type === 'domain';
      })
      .map(item => ({
        data: {
          id: item.data.id,
